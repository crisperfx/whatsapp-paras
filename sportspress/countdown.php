...
if (isset($show_status) && $show_status) {
	...
	$statuses = apply_filters(
		...
		array(
			'ok'        => esc_attr__('On time', 'kester'),
			'tbd'       => esc_attr__('TBD', 'kester'),
			'postponed' => esc_attr__('Postponed', 'kester'),
			'cancelled' => esc_attr__('Canceled', 'kester'),
		)
	);
...
?>
<div class="sp-template sp-template-countdown rts-latest-match-inner">
	...
	if (is_array($teams)) {
		foreach ($teams as $team) {
			if (has_post_thumbnail($team)) {
				$logo_html = '';
				if ($link_teams) {
					$logo_html = '<a class="team-logo" href="' . esc_url(get_post_permalink($team)) . '" title="' . esc_attr(get_the_title($team)) . '">'
						. get_the_post_thumbnail($team, 'large')
						. '</a>';
				} else {
					$logo_html = get_the_post_thumbnail($team, 'large', ['class' => 'team-logo']);
				}
				$logos[] = $logo_html;
			}
		}
	...
	<div class="event-logos-desktop">
		...
		if (is_array($teams)) {
			...
			foreach ($teams as $team) {
				...
				if (has_post_thumbnail($team)) {
					if ($link_teams) {
						echo '<a class="team-logo logo-' . ($i % 2 ? 'odd' : 'even') . '" href="' . esc_url(get_post_permalink($team)) . '" title="' . esc_attr(get_the_title($team)) . '">' . get_the_post_thumbnail($team, 'large') . get_the_title($team) . '</a>';
					} else {
						echo get_the_post_thumbnail($team, 'large', ['class' => 'team-logo logo-' . ($i % 2 ? 'odd' : 'even')]);
					}
				}
			}
		}
	...
	<!-- Mobiel (logo's met VS) -->
	<div class="event-logos-mobile">
		<?php if (!empty($logos)) echo implode('<span class="vs">VS</span>', $logos); ?>
	</div>
</h3>
		...
		<div class="event-details">
			<div class="live"><?php
								echo esc_html__('Live', 'kester'); ?></div>
			...
			?>
			 <?php endif; ?>">
				<time datetime="<?php echo esc_attr($post->post_date); ?>" <?php
					...
					<span><?php echo wp_kses_post(sprintf('%02s', $days)); ?> <small>D.</small></span>
					<span><?php echo wp_kses_post(sprintf('%02s', $h)); ?> <small><?php esc_attr_e('U.', 'kester'); ?></small></span>
					<span><?php echo wp_kses_post(sprintf('%02s', $i)); ?> <small><?php esc_attr_e('min.', 'kester'); ?></small></span>
					<span><?php echo wp_kses_post(sprintf('%02s', $s)); ?> <small><?php esc_attr_e('sec.', 'kester'); ?></small></span>
				</time>
			</p>
		</div>
	</div>

...
if ($show_nationality && $nationalities && is_array($nationalities)) :
	...
	endforeach;
	$data[esc_attr__('Nationaliteit', 'kester')] = implode('<br>', $values);
endif;
...
if ($show_current_teams && $current_teams) :
	...
	foreach ($current_teams as $team) :
		...
		$teams[] = $team_name;
	endforeach;
	$data[esc_attr__('Huidige Ploeg', 'kester')] = implode(', ', $teams);
endif;
...
if ($show_past_teams && $past_teams) :
	...
	foreach ($past_teams as $team) :
		...
		$teams[] = $team_name;
	endforeach;
	$data[esc_attr__('Past Teams', 'kester')] = implode(', ', $teams);
endif;
...
$output = '<div class="sp-list-wrapper">' .
	'<ul class="sp-staff-details">';

foreach ($data as $label => $value) :

	$output .= '<li><span class="key">' . $label . '</span><span class="value">' . $value . '</span></li>';

endforeach;

$output .= '</ul></div>';
?>
<div class="sp-template sp-template-staff-details sp-template-details staff-details-single">

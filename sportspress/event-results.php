...
if ( has_post_thumbnail( $team_id ) ) {
    // featured image van het team ophalen op formaat 50x50
    $team_logo = get_the_post_thumbnail( $team_id, array(50, 50), array(
        'class' => 'sp-team-logo',
        'style' => 'width:50px;height:50px;object-fit:contain;'
    ) );
}

// Indien geen logo, fallback naar korte naam
if ( empty( $team_logo ) ) {
    $team_logo = sp_team_short_name( $team_id );
}

// Optioneel klikbaar maken
if ( $link_teams && sp_post_exists( $team_id ) ) {
    $team_logo = '<a href="' . get_post_permalink( $team_id ) . '">' . $team_logo . '</a>';
}

$table_rows .= '<td class="data-name">' . $team_logo . '</td>';

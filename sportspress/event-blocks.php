		...
		if ( $paginated ) {
			...
			<tbody>
				...
				foreach ( $data as $event ) :
                    ...
                    $is_vriendschappelijk = get_post_meta( $event->ID, '_sp_event_vriendschappelijk', true ) === '1';
...
if ( $is_vriendschappelijk ) :
    // Haal waarde uit custom veld _sp_event_type_voetbal
    $event_type_voetbal = get_post_meta( $event->ID, '_sp_event_type_voetbal', true );
    if ( ! empty( $event_type_voetbal ) ) :
        echo '<div class="sp-event-league">' . esc_html( $event_type_voetbal ) . '</div>';
    endif;
    // Altijd het label Vriendschappelijk tonen
    echo '<div class="sp-event-season">Vriendschappelijk</div>';
else :
    // Normale league weergave
    if ( $show_league ) :
        $leagues = get_the_terms( $event, 'sp_league' );
        if ( $leagues ) :
            $league = array_shift( $leagues );
            echo '<div class="sp-event-league">' . wp_kses_post( $league->name ) . '</div>';
        endif;
    endif;

    // Normale season weergave
    if ( $show_season ) :
        $seasons = get_the_terms( $event, 'sp_season' );
        if ( $seasons ) :
            $season = array_shift( $seasons );
            echo '<div class="sp-event-season">' . wp_kses_post( $season->name ) . '</div>';
        endif;
    endif;
endif;
?>
